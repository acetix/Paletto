...
export default function AppCore() {
  ...
  function downloadSvg() {
    ...
    source.onload = () => {
      ...
      if (!context) { notify('Could not prepare the SVG download.'); return }
      ...
      const link = document.createElement('a'), url = URL.createObjectURL(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }))
      link.href = url; link.download = `${imageLabel.replace(/[^a-z0-9-_]+/gi, '-').replace(/^-|-$/g, '') || 'palette-image'}.svg`; link.click(); URL.revokeObjectURL(url); notify('SVG download started.')
    }
    source.onerror = () => notify('Could not convert this image to SVG.')
    source.src = image
  }
...
function PrivacyPage() {
  return <div className="privacy-page"><nav className="navbar topbar"><div className="container-fluid app-container px-0"><a className="navbar-brand brand-mark" href="/"><span className="brand-icon"><i /><i /><i /><i /></span><span>Palette<span className="brand-period">.</span></span></a><a className="back-link" href="/">← Back to Palette</a></div></nav><main className="privacy-content"><div className="eyebrow"><ShieldCheck size={13} /> YOUR PRIVACY MATTERS</div><h1>Privacy, in plain language.</h1><p className="privacy-updated">Last updated September 23, 2026</p><p>Palette is a free colour palette tool by acetix. We designed it to work without an account and to keep your images on your device.</p><h2>Your images &amp; palettes</h2><p>Images you choose are read and processed locally in your browser to extract colours. Palette does not upload your images to a server. Palettes you save are stored in your browser's local storage and remain on this device. Clearing browser data will remove them.</p><h2>Information we collect</h2><p>Palette does not ask for personal information, create user accounts, or use advertising cookies. The hosting provider may process standard technical request data to deliver this website. We do not sell personal data.</p><h2>Advertising</h2><p>Palette currently does not display advertising. If advertising is introduced in the future, we will update this notice and provide appropriate disclosures and controls in line with applicable policies and law.</p><h2>External links</h2><p>Links to acetix.xyz and your email app are governed by the privacy practices of those services. For acetix's broader policies, visit <a href="https://acetix.xyz/privacy" target="_blank" rel="noreferrer">acetix.xyz/privacy</a>.</p><h2>Contact</h2><p>Questions about this notice? Write to <a href="mailto:acetix.team@gmail.com">acetix.team@gmail.com</a>.</p><a className="btn btn-dark privacy-home" href="/">Back to your palette <ArrowRight size={15} /></a></main><footer className="site-footer"><div className="container-fluid app-container footer-inner"><span className="copyright">© 2026 acetix · Palette</span><div className="footer-links"><a href="https://acetix.xyz/privacy" target="_blank" rel="noreferrer">acetix privacy</a><a href="mailto:acetix.team@gmail.com">Contact</a></div></div></footer></div>
}
